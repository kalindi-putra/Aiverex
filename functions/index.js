...
exports.linkedInAuth = onRequest(async (req, res) => {
  try {
    const { code } = req.query; // Get the LinkedIn authorization code from the query parameter
...
exports.submitCode = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const uploadParams = {
      Bucket: BUCKET_NAME,
      Key: `user-${userId}-code-${Date.now()}.txt`, // Store with a unique key based on userId and timestamp
      Body: codeContent, 
